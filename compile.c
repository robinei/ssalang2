  ...
  switch (basenode->tag) {
    ...
    case AST_TAG_IF: {
      ...
      if (static_eval || node->is_inline) {
        ...
        assert(cond.type == TY_BOOL);
        irgen_branch(gen, cond, then_block, else_block ? else_block : exit_block);
        ...
        result = irgen_phi(gen, phi, then_result.type);
      }
      break;
    }
    ...
    case AST_TAG_WHILE: {
      ...
      if (static_eval || node->is_inline) {
        ...
        assert(cond.type == TY_BOOL);
        irgen_branch(gen, cond, body_block, exit_block);
    ...
    case AST_TAG_CONT: {
      ...
      ScopeSlot *scope = ctx->scopes + node->scope_index;
      if (node->is_static) {
        scope->value_static = ast_compile(node->value, true, ctx);
        longjmp(scope->jump_buffer, basenode->tag);
      }
      else if (basenode->tag == AST_TAG_BREAK) {
        assert(scope->break_target);
        IrInstr value = ast_compile(node->value, false, ctx);
        irgen_upsilon(gen, ctx->block, scope->value_phi, value);
        irgen_jump(gen, scope->break_target);
      }
      else {
        assert(scope->cont_target);
        irgen_jump(gen, scope->cont_target);
      }
      break;
    }
